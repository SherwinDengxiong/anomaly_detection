...
def test():
    ...
    featureroot = 'testing_feature_multi'


    if not os.path.exists(featureroot + '/'):
        os.makedirs(featureroot)
    dirname='cityscene_test'

    save_dir = os.path.join(featureroot, dirname)

    trainpath = root
    a = sorted(os.listdir(trainpath))
    for videoname in tqdm(a, total=len(a)):
        ...
        # result_file = os.path.join(featureroot, name + '.npy')
        ...
        if (framenum < (segnum + frameperseg)):
            for i in range(segnum):
                ...
                # torch.save(newfeature, os.path.join(filepath, name + "_" + str(i)))

                np.save(os.path.join(filepath, name + "_" + str(i)), features)
            progress = progress + 1
            print(name)
            continue
        else:
