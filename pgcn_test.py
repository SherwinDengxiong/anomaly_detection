...
def runner_func(dataset, state_dict, stats, gpu_id, iou_dict, dis_dict):
    ...
    for index in tqdm(range(len(dataset)), total=len(dataset)):
        ...
        val = (rel_props.numpy(), act_scores.cpu().numpy(), comp_scores.cpu().numpy(), reg_scores.cpu().numpy())
        key = dataset.video_list[index].id
...
if __name__ == '__main__':
    ...
    out_dict=runner_func(dataset, base_dict, stats, gpu_list[0],  iou_dict, dis_dict)
    ...
    if args.save_scores is not None:
        ...
        pickle.dump(out_dict, open(args.save_scores, 'wb'), pickle.HIGHEST_PROTOCOL)

    if args.save_raw_scores is not None:
        # save_dict = {k: v[-1] for k, v in out_dict.items()}
        import pickle

        pickle.dump(out_dict, open(args.save_raw_scores, 'wb'), pickle.HIGHEST_PROTOCOL)
